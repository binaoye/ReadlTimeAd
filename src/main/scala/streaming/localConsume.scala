...
import streaming.CosumeKafka._
import utils.ThrowablePrint

import scalaj.http.Http

/**
  * Created by root on 2017/9/6.
  */
object localConsume {
  ...
  def main(args: Array[String]): Unit = {
    val logger = Logger.getLogger("local kafka cosume")
    ...
    val chatHist = chatSend.foreachRDD(r=>{
      ...
      println("调用模型进行预测")
      println("关键词转数组格式")
      var wordArrayDataFrame = new Tokenizer()
        .setInputCol("words")
        .setOutputCol("text")
        .transform(rowDF)
